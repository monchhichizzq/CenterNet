...
class CenterNet:
    def __init__(self, input_shape, num_classes, backbone='resnet18', max_objects=100, mode="train", num_stacks=2):
    ...
    def __call__(self, *args, **kwargs):
        backbone = ResNet18(self.image_input)
        y1, y2, y3 = centernet_head(backbone, self.num_classes)
        model = Model(inputs=[self.image_input], outputs=[y1, y2, y3])
        model.summary()
        return model
